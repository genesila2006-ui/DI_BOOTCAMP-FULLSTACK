import random

def get_words_from_file(file_path):
    """Opens a file, reads its content, and returns a list of words."""
    with open(file_path, "r", encoding="utf-8") as f:
        content = f.read()
        words = content.split()
    return words

def get_random_sentence(length, file_path="sowpods.txt"):
    """Generates a random lowercase sentence of a specified word length."""
    words = get_words_from_file(file_path)
    selected_words = [random.choice(words) for _ in range(length)]
    sentence = " ".join(selected_words).lower()
    return sentence

def main():
    """Handles program setup, user input, and validation."""
    print("Welcome! This program generates a random sentence of a length you specify.")
    
    user_input = input("Enter the desired sentence length (between 2 and 20): ")
    
    try:
        length = int(user_input)
        if 2 <= length <= 20:
            sentence = get_random_sentence(length)
            print(f"\nGenerated Sentence:\n{sentence}")
        else:
            print("Error: The length must be between 2 and 20 inclusive.")
    except ValueError:
        print("Error: Invalid input. Please enter a valid integer.")

if __name__ == "__main__":
    main()


# Exercise 2: Working with JSON

import json

sampleJson = """{ 
   "company":{ 
      "employee":{ 
         "name":"emma",
         "payable":{ 
            "salary":7000,
            "bonus":800
         }
      }
   }
}"""

# Step 1: Load the JSON string into a Python dictionary
data = json.loads(sampleJson)

# Step 2: Access and print the nested "salary" key
salary = data["company"]["employee"]["payable"]["salary"]
print(f"Salary: {salary}")

# Step 3: Add the "birth_date" key to the "employee" dictionary
data["company"]["employee"]["birth_date"] = "1995-04-12"

# Step 4: Save the modified dictionary to a JSON file
with open("modified_employee.json", "w", encoding="utf-8") as f:
    json.dump(data, f, indent=4)

print("Modified JSON saved successfully to 'modified_employee.json'.")