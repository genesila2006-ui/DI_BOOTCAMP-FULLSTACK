...
class Text:
	...
	def word_frequency(self, word) -> int | None:
...
class TextModification(Text):
	...
	def remove_stop_words(self):
		...
		self.text = " ".join(
			word
			for word in words
			if word.lower().strip(string.punctuation) not in self.STOP_WORDS
		)
		return self.text
...
if __name__ == "__main__":
	text = Text("The cat sat on the mat. The cat was happy.")
	print("Frequency of 'cat':", text.word_frequency("cat"))
	print("Most common word:", text.most_common_word())
	print("Unique words:", text.unique_words())

	modified_text = TextModification("The cat is happy! #python")
